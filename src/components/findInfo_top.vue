  ...
  methods: {
    ...
    searchFood() {
      if (this.keyword === "") {
        ...
        this.$emit("sousuocaiming", { name: this.keyword });
        this.finding = false;
        this.$refs.el.blur();
    ...
    biaoqiancaiming(e) {
      ...
      this.$emit("biaoqiancaiming", { name: name });
      global.historyFood.push({ name: name });
      global.historyFood = global.quchong(global.historyFood);
      localStorage.setItem("history", JSON.stringify(global.historyFood));
...
.home_top {
  ...
  .searchConetnt {
    ...
    .hotFind {
      ...
      .findItem:nth-child(1) {
        border: 1px solid #cd8f7d;
        color: #cd8f7d;
      }
      .findItem:nth-child(2) {
